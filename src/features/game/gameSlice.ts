...
export const gameSlice = createSlice({
  ...
  initialState,
  reducers: {
    setIsChallenger: (state, action: PayloadAction<boolean>) => {
      state.isChallenger = action.payload
    },
    ...
    setOpponentGameState: (state, action: PayloadAction<GameState>) => {
      if (state.phase.type === 'choose-going-first' && action.payload.phase.type === 'choose-going-first' && action.payload.phase.status === 'ok') {
        state.phase = {
    ...
    acknowledgePhaseChangeWasReceived: (state, action: PayloadAction<GamePhaseState>) => {
      if (action.payload.type === state.phase.type) {
        state.phase = {
          ...state.phase,
          acked: true
        };
